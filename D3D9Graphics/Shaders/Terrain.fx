                   ...
                   float2 tex0: TEXCOORD0)
{
	...
	outVS.shade = saturate(max(0.0f, dot(normalW, gLightDir)) + 0.3f);
...
//--------------------------------------------------------------------------------------
VS_OUTPUT RenderSceneVS(float3 inPositionOS  : POSITION0,
...
float3 vInBinormalOS : BINORMAL0)
{
	...
	Out.position = mul(float4(inPositionOS, 1.0f), g_mWorldViewProjection);
	...
	float4 vPositionWS = mul(float4(inPositionOS, 1.0f), g_mWorld);
